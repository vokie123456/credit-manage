...
<body contextpath="<%=request.getContextPath()%>">
   ...
   <%@ include file="/frame/loaning/lend_product_config_detail.jsp"%>
	...
	<!-- 当前放款信息 -->
	<div class="divBgColor">
		<input id="projectId" value="${projectId}" type="hidden" /> <label>当前放款信息<input type="hidden"
			id="lendApplyId1000" value="${la.lendApplyId }" /></label>
	</div>
		...
		<form>
			<c:forEach items="${pastList}" var="dis" varStatus="loop">
				<div style="text-align: center;">
					<label onclick="detailMoney(${loop.index})">第${loop.count }次放款信息</label>
				</div>
				<table class="col3">
					<input type="hidden" id="lendApplyId${loop.index}"
						value="${dis.lendApplyId}" />
					<tr>
	...
	<form id="lendPastForm" method="post">
		...
		<!-- 开户行 -->
			...
			<tr>
				...
				<td class="content">${la.lendBank}</td>


				<!-- 小贷公司银行账号（还款） -->
				<td class="label"><fmt:message key="label.company.lend.cardNo"
						bundle="${lang}" /></td>
				<td class="content">${la.lendCard}</td>
			</tr>
			<tr>
				<td class="label"><fmt:message key="label.lend.lendMoney"
						bundle="${lang}" /></td>
				...
				<td class="label"><fmt:message key="label.lend.lendingTime"
						bundle="${lang}" /></td>
				<td class="content"><fmt:formatDate value="${la.lendDate }"
						pattern="yyyy-MM-dd" /></td>
			</tr>
			<td class="label"><fmt:message key="label.sign.applyAgent"
					bundle="${lang}" /></td>
			<td class="content"><showusername:showusername
					value="${la.lendAgent }"></showusername:showusername></td>
