...
<script type="text/javascript"
	src="<c:url value='/scripts/frame/loanbefore/collateral/collateral_warehouse_detail.js'/>"></script>
</head>
...
<body contextpath="<%=request.getContextPath()%>">
	...
	<div id="otherCollateralDetailDiv">
		<div class="divBgColor">仓单评估</div>

		<input type="hidden" id="warehouse_projectId"
			name="warehouseProjectId" value="${projectId}" /> <input
			type="hidden" name="collateralId" value="${warehouse.collateralId }" />
		...
		<table class="textalignCenter tabcustomPeo" cellpadding="0"
			...
			<tr>
				...
				<td class="content tabBorderRight" colspan="3"><bizoptionname:showname biztypekey="1153"
						value="115311" name="collateralType" /></td>
			</tr>
			<tr>
				<td class="label BgColor">他证号：</td>
				<td class="content">${warehouse.itCardNo }</td>
				<td class="label BgColor">公证号：</td>
				<td class="content tabBorderRight">${warehouse.notaryNo }</td>
			</tr>
			<tr>
				<!-- 资产名称 -->
				<td class="label BgColor"><fmt:message
						key="label.assessCar.edit.collateral.name" bundle="${lang}" /></td>
				<td class="content tabBorderRight" colspan="3">${warehouse.collateralName}</td>
			</tr>
			<tr>
				<!-- 贷款金额 -->
				<td class="label BgColor">质押贷款总额(万元)：</td>
				<td class="content">${warehouse.loanMoney}</td>

				<!-- 贷款余额  -->
				<td class="label BgColor">质押贷款余额(万元)：</td>
				<td class="content tabBorderRight">${warehouse.loanBalance}</td>
			</tr>
			<tr>
				<!-- 购买日期 -->
				<td class="label BgColor"><fmt:message
						key="label.assessCar.edit.collateral.buyDate" bundle="${lang}" /></td>
				<td class="content">${warehouse.buyDate}</td>

				<!-- 购买金额-->
				<td class="label BgColor"><fmt:message
						key="label.assessCar.edit.collateral.buyMoney" bundle="${lang}" /></td>
				<td class="content tabBorderRight">${warehouse.buyMoney}</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="content"><bizoptionname:showname biztypekey="1077"
						name="primarycolRight" value="${warehouse.primarycolRight}" /></td>
				<!-- 前顺位质押金 -->
				<td class="label BgColor"><fmt:message
						key="label.assessCar.edit.zycollateral.primarycolAmount"
						bundle="${lang}" /></td>
				<td class="content tabBorderRight">${warehouse.primarycolAmount}</td>
			</tr>
			<tr>
				<td class="label BgColor">本次质押金额(万元)：</td>
				<td class="content">${warehouse.thisAmount}</td>
				<!-- 押品质押率  -->
				<td class="label BgColor"><fmt:message
						key="label.assessCar.edit.zycollateral.collateralRate"
						bundle="${lang}" /></td>
				<td class="content tabBorderRight">${warehouse.collateralRate}</td>
			</tr>
			<tr>
				<!-- 抵押/过户时间 -->
				<td class="label BgColor">质押开始时间：</td>
				<td class="content"><fmt:formatDate value="${warehouse.collateralTime}" pattern="yyyy-MM-dd"/></td>
				<td class="label BgColor">质押结束时间：</td>
				<td class="content tabBorderRight"><fmt:formatDate value="${warehouse.collateralTimeEnd}" pattern="yyyy-MM-dd"/></td>
			</tr>
			<tr>
				<!-- 评估人员 -->
				<td class="label BgColor">评估机构：</td>
				<td class="content">${warehouse.assessName}</td>

				<!-- 评估时间 -->
				<td class="label BgColor">评估时间：</td>
				<td class="content tabBorderRight">${warehouse.assessTime}</td>
			</tr>
			<tr>
				<!-- 评估总价 -->
				<td class="label BgColor">评估总价(万元)：</td>
				<td class="content">${warehouse.assessTotalprice}</td>
				<!-- 资产状态 -->
				<td class="label BgColor">资产状态：</td>
				<td class="content tabBorderRight"><bizoptionname:showname biztypekey="1081"
						name="collateralStatus" value="${warehouse.collateralStatus}" /></td>
			</tr>
			<tr>
				<!-- 备注 -->
				<td class="label BgColor"><fmt:message
						key="label.assessCar.edit.collateral.remark" bundle="${lang}" /></td>
				<td colspan="3" class="content tabBorderRight">${warehouse.remark}</td>
			</tr>
		</table>
		<!-- 共同所有人 -->
