...
<script type="text/javascript"
	src="<c:url value='/scripts/frame/loaning/push_manager.js?v='/><c:out value='${now.getTime()}'/>"></script>
<script type="text/javascript"
	...
	<form id="lendListForm" method="post">
		<table>
			<tr>
				...
				<td width="150">
					<select id="productMaxId" name="productMaxId"  onchange="selectProductMin(this,'productSubId')" style="width:128px;">
						<option value="">请选择</option>
					</select>
				</td>
				<%--<td class="label1">产品小类：</td>--%>
				<%--<td width="150">--%>
					<%--<select id="productSubId" name="productSubId" style="width:128px;">--%>
						<%--<option value="">请选择</option>--%>
					<%--</select>--%>
				<%--</td>--%>
				...
				<td class="label1">推标时间：</td>
				<td class="timerTd">
					<input  loxiaType="date" name="pushSuccessTimeStart" checkmaster="setMinDate" readonly="readonly"/>
					<span>到</span>
					<input loxiaType="date" name="pushSuccessTimeEnd" checkmaster="setMaxDate" readonly="readonly"/>
				</td>
			</tr>
			<tr>
				...
				<td width="150">
                    ...
                    <select  id="lendMoneyStatus" name="lendMoneyStatus" style="width:128px;">
                    	<option value="">请选择</option>
				...
				<td width="250">
					<select name="channelCode" style="width:212px;">
						<option value="">请选择</option>
						<c:forEach items="${ fundList }" var="fml">
							<option value="${ fml.channelCode }" <c:if test="${ fml.channelCode == '210001'}">selected</c:if>>${ fml.channelName }</option>
						</c:forEach>
					</select>
				</td>
			</tr>
		...
		<div class="buttonlist buttonDivWidth">
			...
			<button type="button" id="contractCancel" loxiaType="button" class="confirm" title="tip:<fmt:message key="tip.sys.audit.pushrecruiment" bundle="${lang}"/>"/>
			存管合同撤回
			</button>
			...
			<button type="button" id="expertTBExcel" loxiaType="button" class="confirm" title="tip:<fmt:message key="label.export.credit.butExport" bundle="${lang}"/>">
				导出
			</button>
		</div>
