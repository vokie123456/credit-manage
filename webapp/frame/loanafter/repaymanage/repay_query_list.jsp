...
<script type="text/javascript" src="<c:url value='/scripts/frame/loanafter/repaymanage/repay_query_list.js?v='/><c:out value='${now.getTime()}'/> "></script>
...
<div class="tabSlide">
	<ul class="control clearfix">
		<li class="border"> <a href="javascript:;" id="noReply" >代扣查询</a></li>
		<li><a href="javascript:;" id="alreadyReply">主动还款</a></li>
	</ul>
	...
	<form id="qryfrm" action="" method="POST">
		<table>
			<tr>
				...
				<td class="label">报盘时间：</td>
				<td>
					<input id="" class="style_td" name="startDate" loxiaType="date" trim="true" />
					—
					<input id="" class="style_td" name="endDate" loxiaType="date" trim="true" />
				</td>
				<td class="label">资金渠道：</td>
				<td class="seleTd">
					<bizselect:bizselect biztypekey="2100" id="platformCode" name="platformCode" isChoose="true" />
					<!-- <select>
						<option>合同状态</option>
					</select> -->
				</td>
	  ...
	  <table class='findList-table' text-align:center;cellpadding=0; cellspacing=0; border=1; style='text-align:center;'>
		  ...
		  <tbody id="repaymentSearch_dv">
		 	<div style="padding-bottom: 10px;">扣款结果：
				<select id="transStatusCode" name="transStatusCode">
...
<script type="text/javascript">
    function test(){
        var code=$j('#transStatusCode').val();
        console.log(code);
        transDetailQuery(code);
    }
