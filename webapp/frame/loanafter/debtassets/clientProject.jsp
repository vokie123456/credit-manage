...
<script type="text/javascript" src="<c:url value='/scripts/frame/loanafter/debtassets/clientProject.js'/>"></script>
<title>抵债资产列表</title>
</head>
<body contextpath="<%=request.getContextPath()%>"> 
	<div>
			...
			<table>
				<tr>
				   <td class="label">客户名称：</td>
					...
					<td><select id="cardType" name="cardType" class="ui-loxia-default ui-corner-all" /></td>
				    <td class="label">证件号：</td>
					<td class="width150"><input loxiaType="input" trim="true" name="cardNo" /></td>
				</tr>
				<tr>
					<!-- 项目编号-->
					<td class="label"><fmt:message key="label.audit.taskquery.project_code" bundle="${lang}" /></td>
					<td width="150"><input name="projectCode" loxiaType="input" trim="true" /></td>
					<td class="label">客户经理：</td>
					<td width="150">
						<user:user bizHall="${bizHalls }" roleCode="0101,0102" userId="${userId }" compCode="${compCode }"
							id="broker" name="broker" isChoose="true" />
						</td>
					<!--  
		...
		<div class="buttonlist buttonDivWidth">
			...
			<button type="reset" loxiaType="button" class="confirm" title="tip：<fmt:message key="tip.sys.apply.reset" bundle="${lang}"/>" id="resets">
				<fmt:message key="button.reset" bundle="${lang}" />
			</button>
		</div>
		<%@include file="/common/paginationInit.jsp"%>
		<table id="tbl-debtlist"></table>
		<div id="pager"></div>
	</div>
</body>
</html>