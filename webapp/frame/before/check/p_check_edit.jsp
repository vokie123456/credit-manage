...
<script type="text/javascript"
	src="<c:url value='/scripts/frame/before/check/p_check_edit.js'/>"></script>
		<script type="text/javascript"
	src="<c:url value='/scripts/frame/before/common/prev_opinion.js'/>"></script>

<form id="pCheckForm" method="post">
...
<input type="hidden" id="checkUserName"  value="${userInfo.userName }">
	<table  id="addCheckOpinions"  class="col6">
		...
		<c:if test="${flag eq 'edit' }">
				...
				<td class="label"></td>
					<td class="content">
					<input name="opinionId"  type="hidden" value="${col.opinionId}" />
					<input name="taskId"  type="hidden" value="${col.taskId}" />
