...
<script type="text/javascript"
	src="<c:url value='/scripts/frame/before/checkcomm/e_checkcomm_edit.js'/>"></script>
<script type="text/javascript"
	src="<c:url value='/scripts/frame/before/common/prev_opinion.js'/>"></script>
<form id="eCheckCommForm" method="post">
...
<form>
	...
	<c:choose>
		<c:when test="${ifAllot == '100111'}">
			<table class="col3">
				...
				<tr>
					<td colspan="4"  style="padding-left: 10%;"  class="borBottom">
					...
					<td colspan="4" style="padding-left:10%;">
						<table style="width: 100%; border: 1px solid #dcdcdc;">
							<tr>
								...
								<td
									style="font-size: 14px; width: 80%; border-left: 1px solid #dcdcdc; padding-left: 10px;">${ucx:unCleanXSS(co.opinionDetail) }</td>
							</tr>
						</table>
					</td>
				</tr>
			</c:if>
			<c:if test="${userInfo.userId==co.userId && co.checkType==checkType}">
		...
		<c:if test="${flag eq 'edit' }">
			<tr>
				<td colspan="4" style="padding-left: 10%"><input
					...
					name="taskId" type="hidden" value="${col.taskId}" /> <textarea
						class="cleditor" id="checkOpinion" name="checkOpinion" rows="4">${ucx:unCleanXSS(col.opinionDetail) }</textarea>
				</td>
			</tr>
		</c:if>
		<c:if test="${flag ne 'edit' }">
			<tr>
				<td colspan="4" style="padding-left: 10%"><textarea rows="4"
						class="cleditor" name="checkOpinion"></textarea></td>
			</tr>
		</c:if>
	</table>
	...
	<table class="col3">
		...
		<tr>
			<td colspan="4" style="padding-left: 10%"><textarea rows="4"
					class="cleditor" id="chairmanOpinion" name="chairmanOpinion">${ucx:unCleanXSS(check.chairmanOpinion) }</textarea></td>
		</tr>
	</table>
</form>
<div id="operateOrder" class="buttonlist buttonDivWidth">
	<input type="button" class="confirm" id="tempSave" value="暂存"
		loxiaType="button" />
</div>
