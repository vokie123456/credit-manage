...
<script type="text/javascript"
	src="<c:url value='/scripts/frame/before/apply/p_apply_edit.js'/>"></script>
	<script type="text/javascript"
	src="<c:url value='/scripts/frame/before/common/prev_opinion.js'/>"></script>

<form id="pApplyForm" method="post" enctype="multipart/form-data"  target="temp" >
	<input type="hidden" id="projectStauts"
		name="clientProject.projectStauts" /> <input type="hidden"
		id="clientType" name="clientBasic.clientType" value="100311" /> <input
		type="hidden" id="applyId" name="applyId"
		value="${applyBasic.applyId}" />
	...
	<table class="textalignCenter tabcustomPeoNoborder">
		<tr>
			...
			<td class="content"><c:if test="${not  empty   projectId }">
				...
				<c:if test="${not  empty   projectId }">disabled="disabled"</c:if>>
					<c:forEach items="${compWorkflowVos }" var="compWorkflowVo">
						<option value="${compWorkflowVo.wfCode }"
							<c:if test="${applyBasic.wfCode==compWorkflowVo.wfCode}"> selected="true"</c:if>>${compWorkflowVo.wfName }</option>
					</c:forEach>
			</select></td>
			<td class="label"></td>
			<td class="content"></td>
	...
	<table class="textalignCenter tabcustomPeoNoborder">
		<tr>
			...
			<td class="content"><input id="loanHopeMoney"
				...
				trim="true" mandatory="true" name="loanHopeMoney" /><label
	...
	<table  class="textalignCenter tabcustomPeoNoborder"  cellpadding="0"
		...
		<tr>
			...
			<td class="content">
			<input type="hidden"   name="clientBasic.cardType"  value="${applyBasic.cardType}" /> 
			<bizoptionname:showname biztypekey="1007"  value="${applyBasic.cardType}" ></bizoptionname:showname></td>
